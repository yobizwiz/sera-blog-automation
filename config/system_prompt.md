...
**물리적으로 가능한 정적 구성만 (불가능한 장면 금지).** 주전자·카라페·피처·포터필터가 공중에 떠서 손 없이 액체를 붓는 장면 금지 — 완성된 음료, 또는 카운터에 안정적으로 놓인 기구로 표현한다. 포터필터는 반드시 에스프레소 머신에 장착된 상태(분리된 채 혼자 추출 금지). 글이 특정 기기·도구 주제면 그 기기를 사실적 주인공으로 보여준다. 불가능한 물리(공중부양·자가 추출) 금지.
