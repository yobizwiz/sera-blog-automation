...
SERA runs differential margins: recognizable BRAND products ("bait") are priced to win search/click demand; in-house "core" products carry the margin. The blog's job is to move brand/bait searchers onto a core product.
- The weekly commercial-investigation share (~60%, usually 4 posts/week) is assigned to the targeting-cluster topics, and each such post's "Our Pick" and CTA is a CORE product direct link. The exact bait brands and core product (with URL) are given in the user message (CTA block + Additional notes) - follow them exactly.
- On comparison and "Best X" posts: put the recognizable BRAND (bait) in the search title, the intro, and the #1 list slot to capture demand; the "Our Pick / Best Value" slot MUST be a core product with a direct product-page link.
- Commercial posts: at least 2 of the (max 6) internal links are direct links to the core product page. How-to / quick-fix (the 40%) stay exactly as before - a collection CTA, no forced product push.
- CTA target: commercial posts link the CTA button to the core PRODUCT page (button text = a natural product-name phrase, NOT a collection name). Hub / how-to / quick-fix keep the COLLECTION CTA.
- Never print price or discount numbers (they change often). Use neutral value language: "our pick", "great value", "best all-rounder".
- Only products named in the notes are core picks; never call any other product "our pick" / "best value". If a core is unavailable, the notes name the substitute.
"""
...
- All links absolute https://se-ras.com/.
...
def _build_user_prompt(*, date, topic, post_type, subtype, cta, hub_links, extra_notes):
    ...
    return (
        "Article publish date: " + date + " (Home Cafe & Baking by SERA)\n"
        "Topic: " + topic + "\n"
        + type_str + "\n"
        ...
        "CTA button text matches its target 1:1 (collection name for a collection CTA; the given product-name phrase for a product CTA), all links https://se-ras.com/, F first/(C) parens, "
