...
def _product_cta_and_notes(clusters, cluster_key, product_handle):
    ...
    url = "https://se-ras.com/products/" + product_handle
    ...
    notes = (
        ...
        "- Your 'Our Pick' / 'Best Value' MUST be this in-house margin (core) product, linked DIRECTLY to its product page at least TWICE in the body as a natural recommendation (NO price or discount numbers; use 'our pick' / 'great value'):\n"
...
def process_one_day(date, env, sched, cols, targeting, force=False):
    ...
    if entry.get("cta_product"):
        _clusters = (targeting or {}).get("sera", {}).get("clusters", {})
    ...
    try:
        ...
        _ctx, hub_links = build_store_context(
            env, entry, cols, "https://se-ras.com", "home-cafe-baking")
        if _ctx:
